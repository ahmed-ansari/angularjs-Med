...
angular.module('forceng', [])
  ...
  .factory('force', function ($rootScope, $q, $window, $http) {
    ...
    // To override default, pass appId in init(props)
      appId = '3MVG9ZL0ppGP5UrDrhNwZbwC75GeotuQfA7sOSo6uxGy5Mam.KF0Qp4YlFJH1XMdItIrTsdkucx43FskQmu5d',
    ...
    function loginWithPlugin() {
      document.addEventListener("deviceready", function () {
        ...
        oauthPlugin.getAuthCredentials(
          function (creds) {
            // Initialize ForceJS
            init({accessToken: creds.accessToken, userId:creds.userId,instanceURL: creds.instanceUrl, refreshToken: creds.refreshToken});
            if (deferredLogin) deferredLogin.resolve();
          },
          function (error) {
            console.log(error);
    ...
    function getUserId() {
      ...
      return (typeof(oauth) !== 'undefined') ? oauth.userId : undefined;
    ...
    function apexrest(pathOrParams) {
      ...
      if (pathOrParams.substring) {
        ...
        if (params.path.substr(0, 18) !== "/services/apexrest") {
          params.path = "/services/apexrest/MedConnectDev" + params.path;
        }
      }
    ...
    function apexrestSFDC(pathOrParams) {
      ...
      if (pathOrParams.substring) {
        ...
        if (params.path.substr(0, 18) !== "/services/apexrest") {
          params.path = "/services/data/v37.0/composite/tree" + params.path;
        }
      }
